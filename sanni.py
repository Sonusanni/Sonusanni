...
bot = telebot.TeleBot('7339531468:AAGSnI7GfoNpwg-Fzu-Bh58JGq3Xn-5u3yc')
...
def show_all_users(message):
    ...
    if user_id in admin_id:
        ...
        response = "Only @Sanni_j_bot Can Run This Command."
    bot.reply_to(message, response)

@bot.message_handler(commands=['owner'])
def show_owner(message):
    response = "👑 Bot Owner: @Sanni_j_bot"  # Replace with the actual owner username
    bot.reply_to(message, response)
...
def handle_bgmi(message):
    ...
    if user_id in allowed_user_ids:
        ...
        response = ("🚫 Unauthorized Access! 🚫\n\n"
                    ...
                    "💬 Chat with an Owner @Sanni_j_bot now and level up your capabilities!\n\n"
...
def welcome_start(message):
    ...
    response = (
        f"🥀Welcome {user_name}!\n\n"
        " For User ID : /id \n\n"
        "👉 Join our official channel - @Sanni_j_bot ✅\n\n"
        "👑 For access: @Sanni_j_bot"
        "👑 OWNER : @Sanni_j_bot"
    )
    bot.reply_to(message, response)
...
def show_access_expiry(message):
    ...
    if user_id in allowed_user_ids:
        ...
        response = (
            ...
            "👉 Contact an Admin or the Owner @Sanni_j_bot for approval.\n"
            "🌟 Become a proud supporter and purchase approval.\n"
            "💬 Chat with an Owner @Sanni_j_bot now and level up your capabilities!\n\n"
            "🚀 Ready to supercharge your experience? Take action and get ready for powerful attacks!"
        )
...
def clear_logs_handler(message):
    ...
    if user_id in admin_id:
        ...
        bot.reply_to(message, "Only @Sanni_j_bot can run this command.")
